...
__object_damage = {
	//Allow dmg process
	private ["_hitpoints","_array","_hit","_selection","_key","_damage","_allFixed","_isVault"];
	_isVault = typeOf _object in DZE_LockedStorage;
...
call {
	...
	if (_type == "accessCode" || _type == "buildLock") exitWith {
		call _object_variables;
	}; 
	if (_type == "objWallDamage") exitWith {
		call _objWallDamage;
	}; 
